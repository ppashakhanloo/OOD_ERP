...
public class MonetaryResourceDAO extends ResourceDAO {
    ...
    public Resource get(String key) {
        try {
            ResultSet rs = myStmt.executeQuery("SELECT * from monetary_resource inner join resource on human_resource.ResourceID = resource.ID");
    ...
    private ArrayList<Resource> listConditional(String cond) {
        String query = "SELECT * from monetary_resource inner join resource on monetary_resource.ResourceID = resource.ID" + (cond == null ? "" : " WHERE " + cond);
    ...
    public ArrayList<Resource> getResourcesByProjectID(String pid) {
        ...
        try {
            ResultSet rs = myStmt.executeQuery("SELECT * from monetary_resource inner join resource on monetary_resource.ResourceID = resource.ID AND ProjectID = "+"'" + pid + "'");
            while(rs.next()){
