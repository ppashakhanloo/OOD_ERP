...
public class ResourceDAO {
    ...
    public boolean update(Resource item) {
        try {
            myStmt.executeUpdate(queryGenerator.update("resource", "resourceStatus",
                    item.getResourceStatus().toString(), "ID = " + item.getID()));
            myStmt.executeUpdate(queryGenerator.update("resource", "isAvailable", (item.isAvailable() ? "1" : "0"),
                    "ID = " + item.getID()));
        } catch (SQLException e) {
            e.printStackTrace();
            return false;
    ...
    public boolean remove(String key) {
        try {
            myStmt.executeUpdate(queryGenerator.delete("resource", "ID = " + key));
        } catch (SQLException e) {
            e.printStackTrace();
            return false;
        }
